		...
		async (discount: DiscountRecord) => {
			...
			try {
				...
				if (!res.ok) {
					...
					throw new Error(
						payload?.error ?? 'Failed to update discount',
					);
				}
				await loadDiscounts();
			} catch (error) {
		...
		async (discount: DiscountRecord) => {
			...
			try {
				...
				if (!res.ok) {
					const payload = await res.json().catch(() => null);
					throw new Error(
						payload?.error ?? 'Failed to delete discount',
					);
				}
				await loadDiscounts();
			} catch (error) {
	...
	const handleCreateDiscount = useCallback(
		async (state: NewDiscountState) => {
			...
			try {
				...
				const maxUsesValue = state.maxUses
					? Number(state.maxUses)
					: null;
				if (
					maxUsesValue !== null &&
			...
			<header className="flex items-start justify-between gap-4">
				<div className="space-y-1">
					...
					<p className="text-muted-foreground text-sm">
						Monitor Midtrans orders, Supabase snapshots, and
						discount rules.
					</p>
				</div>
				...
				<div className="flex items-start justify-between gap-4">
					<div>
						...
						<p className="text-xs text-muted-foreground">
							Data is pulled from Supabase `orders` with the
